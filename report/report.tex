...
\subsection{Python bindings}\label{pyBindings}
...
This algorithm gives an idea of what the program needs to do, but does not describe at all how to do it. Figure \ref{fig:detailedOverallAlgorithm} provides a more detailed overview of how the program will operate. (Figure \ref{fig:detailedOverallAlgorithm} is a bit messy, this is because I was mainly using it to note down some ideas)\\\\
\begin{figure}
	\centering
...
Figure \ref{fig:fetchingValidatingInput}
\begin{figure}[h!]
	\centering
...
\item Autoencoders?
\item Histogram?
\item Simply comparing pixel values (some kind of distance function between images)?
\end{itemize}
...
As I plan on creating a python module from my C++ code, it makes perfect sense to properly document this python module so end-users can use it in more advanced automation scripts if they want. Doing this shouldnt require too much extra effort, just need to write good documentations/examples. This has already been discussed a bit in section \ref{pyBindings}.
...
These variables exist inside the C++ vid2photogrammetry library, Not the GUI. Most of these variables are local to various functions.
\begin{center}
\resizebox{\textwidth}{!}{
...
\end{center}
\subsubsection{GUI variables}
These variables are used to store user input. The values of these variables will be fetched when the start button is pressed.
Most of these variables will be passed into the C++ code, but im not going to copy them into the variables table for the C++ code. All of these variables have the ``type'' column for this reason (type doesnt matter in python because its a weird language, but it does in C++).
\begin{center}
\resizebox{\textwidth}{!}{
\begin{tabular}{|p{5cm}|p{3cm}|p{10cm}|p{10cm}|}
\hline
Name & Type & Description & Validation \\
\hline \hline
inputPath & string & Input video file path, used to open the video which frames are to be extracted from. & Must be an existing file, which the program has read permissions for. Must be of supported video filetypes (any video format openCV can work with).\\
\hline
outputFolder & string & Location extracted frames will be written to. & Must be an existing folder which the program has read/write permissions for.\\
\hline
outputName & string & Used to name extracted frames being output by the program. & Any string should be fine, but length could cause some issues (OS has max filepath length) so it should probably have a maximum length of maybe 64 characters or something.\\
\hline
enableResizing & bool & Checkbox to enable resizing of extracted frames &  \\
\hline
resizeX & unsigned int & Target X size passed to resize function if enableResizing is true. Extracted frames will be resized on the X dimension to this number & Probably shouldnt be bigger than the input image X size. \\
\hline
resizeY & unsigned int & Target Y size passed to resize function if enableResizing is true. Extracted frames will be resized on the Y dimension to this number & Probably shouldnt be bigger than the input image Y size. \\
\hline
enableBlurryRemoval & bool & Checkbox to enable blurry image detection and removal on extracted frames & \\
\hline
blurThreshold & float & Allows the user to control how blurry an image needs to be to be deleted & If using variance of laplace to detect blur, must be greater than 0. \\
\hline
enableOutlierRemoval & bool & Checkbox to enable removal of outlier images (for example, frames which are fully black due to camera being covered) & \\
\hline
outlierThreshold & float & Allows the user to control how much of an outlier an image needs to be to be deleted &  \\
\hline
enableDupeRemoval & bool & Checkbox to enable removal of duplicate images (or very near duplicate images), from where the camera was still for an extended period of time &  \\
\hline
dupeThreshold & float & Allows the user to control how close to other images a ``duplicate'' needs to be to be deleted & Should probably be greater than 0. \\
\hline
\end{tabular}
}
\end{center}
\subsection{Structures}
I cannot think of any structs that could be required right now...
\subsection{Classes}
The C++ code doesnt need any classes, at least I cant think of any it would require. The python code might use one somewhere for the GUI though.
